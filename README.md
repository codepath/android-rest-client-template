...
To simplify, we will declare a separate User class will that will reference the data.  However, by using the `@Embedded` annotation, the fields in this class will be stored as part of the Tweet table.  Room specifically disallows references between two different entities for performance reasons (see https://developer.android.com/training/data-storage/room/referencing-data), and it's easier to manage the data in one table.
...
public class User {
    ...
    @ColumnInfo  // no @PrimaryKey because this is an embedded object
    Long twitter_id;
}
```
...
For the User object, the parsing logic would be:
...
Next, you will need to define the queries by creating a Data Access Object (DAO) class.   We will expose initially
queries to return a Tweet by the post ID, retrieve the most recent tweets, and insert tweets.

```java
The data access object (DAO) layer will be defined
...
We need to define a database that extends `RoomDatabase` and describe which entities as part of this database. We also need to include what data access objects are to be included.  If the entities are modified or additional ones are included, the version number will need to be changed.
...
To save, you will need to create a `Runnable` instance and add the item:
